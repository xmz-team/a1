        ...
        *)
            if [[ "$script_path" == *"usr/local/bin/a1" ]]; then
                ...
                if [ -n "$A1CTL_CALL_MOD" ] && [ "$A1CTL_CALL_MOD" = "true" ]; then
                    ...
                    if [[ "$cmdline" == *"a1"* && "$cmdline" != *"a1ctl"* && "$cmdline" != *"a1module"* ]]; then
                        echo "a1"
...
load_modules_common() {
    ...
    while IFS= read -r module_id; do
        ...
        if [ -z "$module_info" ]; then
            echo "[warn] 模塊 $module_id 不存在於數據庫"
            continue
        fi
        ...
        if [ "$should_load" = false ]; then
            echo "[skip] 模塊 $module_id 目標為 $target，不適用於 $script_type"
            continue
        fi
        ...
        if [ -n "$module_path" ] && [ -f "$module_path" ]; then
            ...
            if [ "$loaded" = false ]; then
                ...
                if [ -n "$first_sh" ]; then
                    source "$first_sh"
                    ((count++))
                    echo "✓ 已加載: $module_id (來自 $first_sh)"
                else
                    echo "[Warn] 模塊文件不存在: $module_id"
                fi
            fi
        else
            echo "[Warn] 模塊文件不存在/沒有: $module_id"
        fi

#!/bin/bash
# In jailbreak ios compile, if you is MacOS or Linux or Windows, please self write build.sh.
set -exuo pipefail
...
export CPPFLAGS="-I$origpwd/libs/libxmz"
...
chmod="$jub/chmod"
mv="$jub/mv"
envsubst="$jub/envsubst"
dpkg_deb="$jub/dpkg-deb"
dpkg_name="$jub/dpkg-name"

source "$origpwd/dependent.ini"
# sudo apt-get update >/dev/null 2>&1
sudo apt install -y $dependent
...
build()
{
		...
		bundle_pid()
		{
				...
				$CXX -fobjc-arc -framework Foundation -framework Security -I. bundle_pid.mm -o bundle_pid
		...
		raise_power()
		{
				...
				$CXX raise_power.cc -o raise_power
...
package()
{
		local src_a1="src/old/a1"
		local src_bin="src/bin"
		local src_gui="src/old/gui"
      ...
      local a1_version="$a1_version"
      local gui_version="$gui_version"
		...
		cp_bin
		cp_gui
		local a1_deb_d="src/old/DEBIANS/DEBIAN.a1"
		local gui_deb_d="src/old/DEBIANS/DEBIAN.gui"
...
main() {
    #init
    #package
    echo "If it has been abandoned, please use the new construction stetlet." >&2
}

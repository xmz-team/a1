...
c++ -fobjc-arc \
    ...
    a1c.mm \
    -o a1 && ldid -S$jb/opt/AD/share/ens/a1c.ens.xml -Hsha1 -Hsha256 -M a1 && chmod 6755 a1
--- end ---
c++ -fobjc-arc \
    -framework UIKit \
    -framework Foundation \
...
#import <AD/test/api/ADlibc++-objc-testapi.hpp>
#import <AD/test/api//ADlibc++-shell-testapi.hpp>
...
- (instancetype)init {
    ...
    if (self) {
        ...
        std::string result = AD::system(findCmd, true);
        if (!result.empty()) result.erase(std::remove(result.begin(), result.end(), '\n'), result.end());
        if (!result.empty() && [[NSFileManager defaultManager] fileExistsAtPath:[NSString stringWithUTF8String:result.c_str()]]) {
            _a1ctlPath = [NSString stringWithUTF8String:result.c_str()];
        } else {
            _a1ctlPath = @"a1ctl";
        }
    }
...
- (NSString *)executeCommand:(NSString *)cmd {
    return [NSString stringWithUTF8String:AD::system(std::string([cmd UTF8String]), true).c_str()];
}
- (BOOL)executeCommandSync:(NSString *)cmd {
    return AD::command::bash(std::string([cmd UTF8String]).c_str()) == 0;
}
...
- (void)setupUI {
    ...
    AD::ui::layout::fill(scroll, self.view);
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    AD::fs::mkdir(a1_path.c_str());
    AD::fs::mkdir(a1_bak_path.c_str());
